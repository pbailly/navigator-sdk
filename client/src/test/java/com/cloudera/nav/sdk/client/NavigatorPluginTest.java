...
public class NavigatorPluginTest {
  ...
  public void testRegisterModels() {
    ...
    mockResponse.setNumModels(3);
    mockResponse.setNumFields(10);
    Map<String, Collection<String>> mockErrs = Maps.newHashMap();
    mockErrs.put("model1", Sets.newHashSet("msg1", "msg2"));
    mockResponse.setErrors(mockErrs);
    when(mockTemplate.exchange(eq("http://nav.cloudera.com:7187/api/v8/models"),
        eq(HttpMethod.POST), any(HttpEntity.class), eq(ModelResponse.class)))
        .thenReturn(new ResponseEntity<>(mockResponse, HttpStatus.OK));
    ModelResponse response = plugin.registerModel(TestMClass.class);
